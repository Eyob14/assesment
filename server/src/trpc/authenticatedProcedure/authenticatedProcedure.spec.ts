...
const routes = router({
  ...
  adminCall: adminProcedure.query(() => 'passed'),
})
...
vi.mock('jsonwebtoken', () => ({
  default: {
    verify: (token: string) => {
      ...
      return { user: { id: 2, email: 'valid@email.com', role: 'user' } }
    },
  },
}))
...
it('should throw an error if user do not have admin role', async () => {
  ...
  await expect(usingValidToken.adminCall()).rejects.toThrow(/admin role/i)
})
